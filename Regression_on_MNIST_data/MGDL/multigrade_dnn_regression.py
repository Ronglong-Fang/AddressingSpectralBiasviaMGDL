...
#------------------------------------------------------compute cost----------------------------------------------------------
def squareloss_cost_L2reg(N, Y, layers_dims, parameters, lambd_W):
    """
    ...
    #compute square loss
    squareloss_cost = np.sum(np.square(Y - N)) / (2*m) 
    ...
    L2reg_cost = 0
    for l in range(1, L):
        L2reg_cost = L2reg_cost + lambd_W/2 *  np.sum(np.square(parameters["W"+str(l)])) 
    
   
    cost = squareloss_cost  + L2reg_cost        # total loss
...
def multigrade_linear_activation_backward_L2reg(dN, cache, lambd_W, activation):
    """
    ...
    dW = np.dot(dZ, N_prev.T) + lambd_W*W             
...
def multigrade_backward_L2reg(Y, N, layers_dims, parameters, caches, lambd_W, activation):
    """
    ...
    for l in reversed(range(1, L)):
        ...
        if l == L-1:
            grads["dN" + str(l-1)], grads["dW" + str(l)], grads["db" + str(l)] = multigrade_linear_activation_backward_L2reg(grads["dN" + str(l)], caches[l-1], 
                                                                                                                             lambd_W, "identity")
        else:
            grads["dN" + str(l-1)], grads["dW" + str(l)], grads["db" + str(l)] = multigrade_linear_activation_backward_L2reg(grads["dN" + str(l)], caches[l-1], 
                                                                                                                             lambd_W, activation)           
...
def multigrade_dnn_model_grade_1(trained_variable, data, layers_dims, lambd_W, opt_parameter, max_learning_rate, min_learning_rate, epochs, activation):
    """
    ...
    cleantrain_rses = []
    validation_rses = []
    cleanvalidation_rses = []
    
    train_costs = []
    cleantrain_costs = []
    validation_costs = []
    cleanvalidation_costs = []
    train_time_record = []
    REC_FRQ_iter = []
    ...
    for i in range(epochs+1):
        ...
        if i%opt_parameter["REC_FRQ"]==0:
            ...
            e_time = time.time()
            train_time_record.append(e_time - s_time)
            
            train_costs.append(squareloss_cost_L2reg(train_predict, data["train_Y"], layers_dims, parameters, lambd_W))
            cleantrain_costs.append(squareloss_cost_L2reg(train_predict, data["cleantrain_Y"], layers_dims, parameters, lambd_W))
            
            validation_costs.append(squareloss_cost_L2reg(validation_predict, data["validation_Y"], layers_dims, parameters, lambd_W))
            cleanvalidation_costs.append(squareloss_cost_L2reg(validation_predict, data["cleanvalidation_Y"], layers_dims, parameters, lambd_W))
            
            train_rses.append(rse(data["train_Y"], train_predict))
            cleantrain_rses.append(rse(data["cleantrain_Y"], train_predict))
            
            validation_rses.append(rse(data["validation_Y"], validation_predict))
            cleanvalidation_rses.append(rse(data["cleanvalidation_Y"], validation_predict))
        ...
        for minibatch in minibatches:    
            ...
            # Backpropagation
            grads = multigrade_backward_L2reg(minibatch_Y, N , layers_dims, parameters, caches, lambd_W, activation)
        
        
            # gradient checking
            #gradient_check_n(layers_dims, parameters,  grads, minibatch_X, minibatch_Y, lambd_W, activation, epsilon=1e-7)
    ...
    trained_variable["mulcleantrain_Y"]  = data["cleantrain_Y"]  - train_predict
    trained_variable["mulvalidation_Y"]  = data["validation_Y"]  - validation_predict  
    trained_variable["mulcleanvalidation_Y"]  = data["cleanvalidation_Y"]  - validation_predict 
    # store cost
    trained_variable["train_costs"].append(train_costs)
    trained_variable["cleantrain_costs"].append(cleantrain_costs)        
    trained_variable["validation_costs"].append(validation_costs)
    trained_variable["cleanvalidation_costs"].append(cleanvalidation_costs)
    # store parameter
    ...
    trained_variable['cleantrain_rse'].append(cleantrain_rses)
    trained_variable['cleanvalidation_rse'].append(cleanvalidation_rses)
    # store record iteration
    trained_variable["REC_FRQ_iter"].append(REC_FRQ_iter)
    # store time        
    trained_variable['train_time'].append(train_time_record)
...
#-------------------------------------------------put all moudel together---------------------------------------------------
def multigrade_dnn_model_grade_ell(trained_variable, data, prev_info, layers_dims, lambd_W, opt_parameter,  max_learning_rate, min_learning_rate, epochs, activation):
    """
    ...
    mulcleantrain_Y = trained_variable["mulcleantrain_Y"] 
    mulcleanvalidation_Y = trained_variable["mulcleanvalidation_Y"] 
    ...
    for i in range(epochs+1):
        ...
        if i%opt_parameter["REC_FRQ"]==0:
            ...
            train_time_record.append(e_time - s_time)
            # Cost function
            train_costs.append(squareloss_cost_L2reg(train_N, multrain_Y, layers_dims, parameters, lambd_W))
            cleantrain_costs.append(squareloss_cost_L2reg(train_N, mulcleantrain_Y, layers_dims, parameters, lambd_W))
            validation_costs.append(squareloss_cost_L2reg(validation_N, mulvalidation_Y, layers_dims, parameters, lambd_W))
            cleanvalidation_costs.append(squareloss_cost_L2reg(validation_N, mulcleanvalidation_Y, layers_dims, parameters, lambd_W))
            ...
            cleantrain_rses.append(rse(data["cleantrain_Y"], train_predict + train_N ))
            cleanvalidation_rses.append(rse(data["cleanvalidation_Y"], validation_predict + validation_N ))            
            REC_FRQ_iter.append(i)
    ...
    trained_variable["mulcleantrain_Y"]  = data["cleantrain_Y"] - prev_info["train_predict"]        
    trained_variable["mulcleanvalidation_Y"]  = data["cleanvalidation_Y"] - prev_info["validation_predict"]  
    # store cost
    trained_variable["train_costs"].append(train_costs)
    trained_variable["validation_costs"].append(validation_costs)
    trained_variable["cleantrain_costs"].append(cleantrain_costs)
