...
def data_setup():
    ...
    # Download the image, take a square crop from the center
    image_path = 'building.png'  # Replace with your image file path
...
def MGDLmodel(opt):
    ...
    test_x, test_y, train_x, train_y = data_setup()
    ...
    picklename = 'results/MGDL_epsilon%s_grade%d_trainpsnr%.4e_testpsnr%.4e.pickle' % (
        str(opt.epsilon), opt.grade, history['train_psnrs'][-1], history['test_psnrs'][-1]
    )
...
def analysis(filepath, LossPsnr_print, Fig_print, grade):
    ...
    for grade in range(1, grade+1):
        ...
        if Fig_print:
            ...
            test_img = pred_imgs
            test_img = test_img.reshape(1356, 2040, 3)
            plt.imshow(test_img)
            plt.show()
